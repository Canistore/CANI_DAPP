...
use canistore_types::file::MAX_FILE_SIZE;
...
pub struct InitArgs {
    ...
    default_admin_user: Option<Principal>, // admin
}
    ...
    match args {
        Some(CanisterArgs::Init(args)) => {
            store::state::with_mut(|b| {
                ...
                if let Some(default_admin_user) = args.default_admin_user {
                    b.managers.insert(default_admin_user);
                }
