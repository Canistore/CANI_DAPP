...
#[allow(dead_code)]
pub fn vec_to_array_32(opt_vec: Option<Vec<u8>>) -> Option<[u8; 32]> {
    opt_vec.and_then(|vec| {
        if vec.len() == 32 {
...
#[allow(dead_code)]
pub fn vec_to_subaccount(opt_vec: Option<Vec<u8>>) -> Option<Subaccount> {
    if let Some(array) = vec_to_array_32(opt_vec) {
        Some(Subaccount(array))
    } else {
        None
    }
}
